...
namespace smartReception
{
    public sealed partial class Reports : Page
    {
            ...
            string filterFloorId, DateTimeOffset? start, DateTimeOffset? end)
        {
            ...
            if (start.HasValue) logUrl += "&access_date=gte." + start.Value.ToString("yyyy-MM-dd");
            if (end.HasValue) logUrl += "&access_date=lte." + end.Value.ToString("yyyy-MM-dd");
            ...
            using (JsonDocument ldoc = JsonDocument.Parse(logJson))
            {
                foreach (JsonElement row in ldoc.RootElement.EnumerateArray())
                {
                    ...
                    if (filterFloorId != null && floorId != filterFloorId) continue;
            ...
            string filterFloorId, DateTimeOffset? start, DateTimeOffset? end)
        {
            ...
            if (start.HasValue) url += "&visit_date=gte." + start.Value.ToString("yyyy-MM-dd");
            if (end.HasValue) url += "&visit_date=lte." + end.Value.ToString("yyyy-MM-dd");
            if (filterFloorId != null) url += "&floor_id=eq." + filterFloorId;
        ...
        private async void ExportExcel_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < headers.Length; i++)
                {
                    ...
                    cell.Style.Fill.BackgroundColor = ClosedXML.Excel.XLColor.FromHtml("#0F172A");
                ...
                for (int r = 0; r < _allRows.Count; r++)
                {
                    ...
                    ws.Cell(r + 2, 1).Style.Font.FontColor = ClosedXML.Excel.XLColor.White;
        ...
        //  NAVIGATION
        //  Rules:
        //    - Every handler guards with GetType() != typeof(X) to avoid
        //      re-navigating to the current page.
        //    - SystemsLogsbtn_Click is a dashboard shortcut to SystemLogs;
        //      it also gets the guard for consistency.
        //    - reportsBacktbtn_Click uses Frame.GoBack() so it respects the
        //      actual back stack rather than hard-coding a destination.
        // ══════════════════════════════════════════════════════════════════════

        private void NavMasterDashboard_Click(object sender, RoutedEventArgs e)
        { if (GetType() != typeof(MasterDashboard)) Frame.Navigate(typeof(MasterDashboard)); }
        ...
        // Shortcut button (e.g. on a dashboard card) — same destination as NavLogs,
        // guard added so double-clicking doesn't reload the page unnecessarily.
        private void SystemsLogsbtn_Click(object sender, RoutedEventArgs e)
        { if (GetType() != typeof(SystemLogs)) Frame.Navigate(typeof(SystemLogs)); }

        // Back button — uses the frame's back stack instead of a hard-coded page.
