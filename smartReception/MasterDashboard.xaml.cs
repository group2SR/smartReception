...
namespace smartReception
{
    public sealed partial class MasterDashboard : Page
    {
        ...
        private void ApplyUnifiedUIFilter()
        {
            ...
            if (ActiveClientsListView != null)
                ActiveClientsListView.ItemsSource = _cachedActiveClients
                    .Where(c => MatchesQuery(query, c.FullName, c.NIN) && MatchesFloor(floorFilter, c.FloorTag)).ToList();

            if (AllLogsListView != null)
                AllLogsListView.ItemsSource = _cachedAllLogs
                    .Where(l => MatchesQuery(query, l.FullName, l.NIN) && MatchesFloor(floorFilter, l.FloorTag)).ToList();

            if (DeletedClientsListView != null)
                DeletedClientsListView.ItemsSource = _cachedDeletedLogs
                    .Where(d => MatchesQuery(query, d.FullName, d.NIN) && MatchesFloor(floorFilter, d.FloorTag)).ToList();
        }
        ...
        private async Task FetchLatestDatabaseLogsAsync()
        {
            try
            {
                var activeTask = FetchActiveClientsAsync();
                var logsTask = FetchAllLogsAsync();
                var deletedTask = FetchDeletedClientsAsync();
        ...
        private async Task<List<ActiveClientDisplay>> FetchActiveClientsAsync()
        {
            string url = App.SupabaseUrl + "/rest/v1/access_logs" +
                         ...
                         "&status=eq.Signed%20In&order=time_in.desc";
            ...
            using (JsonDocument doc = JsonDocument.Parse(json))
            {
                foreach (JsonElement row in doc.RootElement.EnumerateArray())
                {
                    if (!row.TryGetProperty("client", out JsonElement client) ||
                        client.ValueKind == JsonValueKind.Null) continue;
                    ...
                    if (!client.TryGetProperty("floors", out JsonElement floor) ||
                        floor.ValueKind == JsonValueKind.Null) floor = default;
                    ...
                    result.Add(new ActiveClientDisplay
                    {
                        ...
                        FloorTag = floorId == 1 ? "1" : (floorId == 2 ? "2" : "All"),
            ...
            using (JsonDocument doc = JsonDocument.Parse(json))
            {
                foreach (JsonElement row in doc.RootElement.EnumerateArray())
                {
                    ...
                    result.Add(new VisitorLogDisplay
                    {
                        ...
                        TimeInSummary = status == "Signed In"
            ...
            using (JsonDocument doc = JsonDocument.Parse(json))
            {
                foreach (JsonElement row in doc.RootElement.EnumerateArray())
                {
                    ...
                    result.Add(new DeletedClientDisplay
                    {
                        ...
                        FloorName = GetStr(floor, "floor_name") ?? "",
                        FloorTag = floorId == 1 ? "1" : (floorId == 2 ? "2" : "All"),
                        DeletedDateFormatted = DateTime.TryParse(rawDate, out DateTime dt)
                            ? dt.ToString("dd/MM/yyyy") : rawDate,
        ...
        private async Task<string> GetJsonAsync(string url)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                throw new InvalidOperationException(
                    $"Supabase error ({(int)response.StatusCode} {response.ReasonPhrase}): {body}");
            }
        ...
        private static JsonElement FirstIfArray(JsonElement el)
        {
            if (el.ValueKind == JsonValueKind.Array)
                foreach (JsonElement item in el.EnumerateArray()) return item;
            return el;
        }
        ...
        private static string FormatTime(string raw)
        {
            ...
            return TimeSpan.TryParse(raw, out TimeSpan t)
                ? DateTime.Today.Add(t).ToString("hh:mm tt") : raw;
        }

        private static string GetStr(JsonElement el, string prop) =>
            el.TryGetProperty(prop, out JsonElement v) && v.ValueKind != JsonValueKind.Null
                ? v.GetString() : null;

        private static int GetInt(JsonElement el, string prop) =>
            el.TryGetProperty(prop, out JsonElement v) && v.ValueKind == JsonValueKind.Number
                ? v.GetInt32() : 0;
            ...
            if (sender is Button btn && btn.Tag is int logId)
            {
                try
                {
                    ...
                    var req = new HttpRequestMessage(
                        ...
                        App.SupabaseUrl + "/rest/v1/access_logs?log_id=eq." + logId)
                    {
